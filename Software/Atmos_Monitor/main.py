...
def main():
    ...
    while True:
        ...
        border_sprite = displayio.TileGrid(
        ...
        # Timer bar
        sleepbar_bitmap = displayio.Bitmap(8, int(HEIGHT * timer / init_timer), 1)
        sleepbar_palette = displayio.Palette(1)
        sleepbar_palette[0] = 0xFFFFFF
        sleepbar_sprite = displayio.TileGrid(
            sleepbar_bitmap, pixel_shader=sleepbar_palette, x=0, y=0
        )
        screen.append(sleepbar_sprite)

        text = "Temp (C):   {:.2f}".format(bme280.temperature)
        text_area = label.Label(
            terminalio.FONT, text=text, color=0xFFFFFF, x=11, y=11
        )
        screen.append(text_area)

        text = "Humi (%):   {:.2f}".format(bme280.relative_humidity)
        text_area = label.Label(
            terminalio.FONT, text=text, color=0xFFFFFF, x=11, y=int((HEIGHT / 2))
        )
        screen.append(text_area)

        text = "Pres (hPa): {:.2f}".format(bme280.pressure)
        text_area = label.Label(
            terminalio.FONT, text=text, color=0xFFFFFF, x=11, y=HEIGHT - 12
        )
        screen.append(text_area)
